...
class ViewController: UIViewController {
    ...
    private var padWetDryTimer: Timer?
    private var padWetDryTarget = 0.0
    private var padWetDryValue = 0.0
    private var speedRestoreTimer: Timer?
    private var playSliderUpdateTimer: Timer?
    ...
    @objc private func hitTouch(_ sender: TriggerGestureRecognizer) {
    ...
    @objc private func effectPadPan(_ sender: TriggerGestureRecognizer) {
        if sender.state == UIGestureRecognizerState.began {
    ...
    @objc private func stereoSweepTouch(_ sender: TriggerGestureRecognizer) {
        if let audio = audio {
            if sender.state == UIGestureRecognizerState.began || sender.state == UIGestureRecognizerState.changed {
...
private extension ViewController {
    ...
    private func associatedPlayerForView(_ view: UIView) -> AEAudioFilePlayerModule? {
        if let audio = audio {
